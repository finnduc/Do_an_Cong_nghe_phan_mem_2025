...
import { deleteAccount, fetchAccounts, updateAccount } from "@/lib/api/accounts";
...
export default function AccountsTable({ data, totalPages, totalRecords, onSuccess }) {
  ...
  const getNextPage = async (page) => {
    try {
      ...
      toast.error(
        e.message || "An error occurred while fetching the account. Please try again or contact the administrator."
      );
    }
  };
  ...
  const handleEditSubmission = async () => {
    ...
    try {
      await updateAccount(selectedAccount.user_id, formData.username, formData.password);
      setSuccessMessage('The account has been updated successfully.');
      onSuccess();
    } catch (e) {
      toast.error(
        e.message || "An error occurred while updating the account. Please try again or contact the administrator."
      );
    }
  };
  ...
  const handleDeleteSubmission = async () => {
    try {
      ...
      toast.error(
        e.message || "An error occurred while deleting the account. Please try again or contact the administrator."
      );
    }
  };
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg border shadow-md p-4">
            ...
            {successMessage ? (
              ...
              <div>
                Are you sure you want to delete the account with username "{selectedAccount.username}"? <br /> This action can only be undone by admin.
              </div>
            )}
