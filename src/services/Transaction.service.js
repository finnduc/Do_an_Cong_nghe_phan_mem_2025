...
class TransactionService {
  getTransaction = async (payload) => {
    ...
    if (transaction_id) {
      addQuery += " AND CAST(th.header_id AS CHAR) LIKE ?";
      params.push(`${transaction_id}%`);
    } else {
      if (employee) {
        addQuery += " AND e.employee_id = ?";
        params.push(`${employee}`);
      }
      if (partner) {
        addQuery += " AND p.partner_id = ?";
        params.push(`${partner}`);
      }
      if (product_name) {
      ...
      if (priceRangeMax) {
        addQuery += " AND th.total_amount <= ?";
        params.push(priceRangeMax);
      }
      if (priceRangeMin) {
        addQuery += " AND th.total_amount >= ?";
        params.push(priceRangeMin);
      }
    }
