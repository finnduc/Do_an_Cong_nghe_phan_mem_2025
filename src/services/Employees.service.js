...
const {
  findEmployeeByInfo,
  findEmployeeByID,
} = require("../models/repo/employees.repo");

class EmployeeService {
  // Create employee
  createEmployee = async (payload) => {
    const foundEmployee = await findEmployeeByInfo({
      email: payload.email,
      phone: payload.phone,
    });
    ...
    if (foundEmployee[0]) {
      throw new ConflictError(
        "Nhân viên có số điện thoại và email này đã tồn tại!"
      );
    }
  ...
  deleteEmployee = async (payload) => {
    ...
    if (notFoundIds.length > 0) {
      throw new NotFoundError(
        `Không tìm thấy các employee_id sau: ${notFoundIds.join(", ")}`
      );
    }
