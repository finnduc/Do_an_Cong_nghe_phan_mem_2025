...
const {
  ...
  findIdManu,
  findIdProduct,
} = require("../models/repo/parameter.repo");
const {
  findManuStock,
  findCateStock,
  findProductStock,
  findStockByParams,
} = require("../models/repo/stock.repo");
...
class ParameterService {
  createManu = async (payload) => {
    ...
    if (!foundManu[0]) {
      ...
      queryManufacturers = `
            ...
            `;
      result = await executeQuery(queryManufacturers, [
        foundManu[0].manufacturer_id,
      ]);
    } else {
    ...
    return {
      message: !foundManu[0]
        ...
        : "Khôi phục nhà sản xuất thành công",
      data: result,
    };
  };
  ...
  updateManu = async (payload) => {
    ...
    const updateManufacturers = await executeQuery(queryUpdateManufacturers, [
      manufacturer,
      manufacturer_id,
    ]);
  ...
  deleteManu = async (payload) => {
    ...
    const deleteManufacturers = await executeQuery(queryDeleteManufacturers, [
      manufacturer_id,
    ]);
  ...
  createCategory = async (payload) => {
    ...
    if (!foundCategory[0]) {
      ...
      queryCategories = `
                ...
                WHERE category_id = ?;
            `;
      result = await executeQuery(queryCategories, [
  ...
  updateCategory = async (payload) => {
    ...
    const updateCategories = await executeQuery(queryUpdateCategories, [
      category,
      category_id,
    ]);
  ...
  deleteCategory = async (payload) => {
    ...
    const deleteCategories = await executeQuery(queryDeleteCategories, [
      category_id,
    ]);
  ...
  createParameter = async (payload) => {
    ...
    if (foundManufacturer[0].is_deleted || foundCategory[0].is_deleted) {
      throw new BadRequestError(
        "Không thể tạo tham số vì nhà sản xuất hoặc danh mục đã bị xóa!"
      );
    }
    ...
    const existsParameter = await findParametersEx(
  ...
  updateParameter = async (payload) => {
    ...
    if (
      foundParameter[0].manufacturer_id !== foundManufacturer[0].manufacturer_id
    ) {
      const queryUpdate = `
                UPDATE parameters
                SET manufacturer_id = ?
    ...
    const updateProduct = await executeQuery(updateProductName, [
      name_product,
      product_id,
    ]);
  ...
  deleteParameter = async (payload) => {
    ...
    const foundProductStock = await findProductStock(
    ...
    if (foundProductStock[0]) {
      ...
      const remainingStock = await findProductStock(
        foundParameter[0].product_id
      );
      if (remainingStock[0]) {
        throw new ConflictError(
          "Không thể xóa vì tồn tại sản phẩm trong kho với số lượng > 0"
        );
      }
    }
    ...
    const queryUpdateParameter = `
            ...
            WHERE parameter_id = ?;
        `;
    const updateParameter = await executeQuery(queryUpdateParameter, [
      parameter_id,
    ]);
    ...
    const queryUpdateProduct = `
            ...
            WHERE product_id = ?;
        `;
    const updateProduct = await executeQuery(queryUpdateProduct, [
      foundParameter[0].product_id,
    ]);
  ...
  searchParameter = async (payload, query) => {
    ...
    return await executeQuery(querySearch, [
      `%${search}%`,
      `%${search}%`,
      `%${search}%`,
    ]);
